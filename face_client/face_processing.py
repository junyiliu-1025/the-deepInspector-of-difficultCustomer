...
from face_function_client import images_path, sub_wd, sub_hi, text_wd, text_hi, bg, face_wd , face_hi , data_wd, data_hi
...
def deal_window(path, images, root, frm, cnv):
    ...
    if messagebox.askyesno("Deal", "確定處理麼?", parent=root):
        ...
        if(images):
            ...
            textbox.place(relx=0.67, rely=0.00) 
            ## Frame in textbox
            ...
            for file in images:
                ...
                # 顯示圖片資訊
                textbox.insert(END," I found {} face(s) in {}.\n".format(len(face_locations), file))

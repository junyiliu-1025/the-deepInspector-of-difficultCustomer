...
class Calendar:
    ...
    def __init__(s, point=None, position=None):
        ...
        #日歷暫留時間
        #s.master.after(3000, s._main_judge)
        s.master.deiconify()
        s.master.focus_set()
    ...
    def __place_widgets(s):
        ...
        ttk.Button(bframe, text="確 定", width=6, command=lambda: s._exit(True)).grid(row=0, column=0, sticky='ns',
                                                                                    padx=20)
...
if __name__ == '__main__':
    ...
    root.geometry('%dx%d+%d+%d' % (width + 50, height + 10 , x, y))  # 窗口位置居中
    ...
    tk.Button(root, text='選擇日期:', command=date_str_gain).place(x=0, y=0, relx=1 / 20, rely=1 / 6, relwidth=4 / 20,
                                                             relheigh=2 / 3)
    root.mainloop()
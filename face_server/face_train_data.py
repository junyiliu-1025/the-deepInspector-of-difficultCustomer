...
def get_img_list(dirname,flag=0):
    rootdir= os.path.abspath('./faces/train/'+dirname+'/')
...
def read_img(list,flag=0):
    for i in range(len(list)-1):
         if os.path.isfile(list[i]):
             #had been changed
             images.append(cv2.cvtColor(cv2.imread(list[i]), cv2.COLOR_BGR2GRAY).flatten())
             labels.append(flag)

read_img(get_img_list('okes'),[0,1])
read_img(get_img_list('unokes'),[1,0])
